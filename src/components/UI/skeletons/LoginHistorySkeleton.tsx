  ...
  return (
    <div className={"bg-(--background-container) p-2 rounded-md"}>
      ...
      <TableContainer>
        <table
          className={
            "w-full table-fixed border-separate border-spacing-0 bg-(--table-bg) "
          }
        ...
        >
            ...
            <tr className="block xl:table-row bg-white/20 hover:bg-white/35 transition rounded-md">
              {tableDocumentMap.map((item) => (
                <th
                  scope={"col"}
                  className={
                    "sticky top-0 bg-(--table-row) hover:bg-(--table-row-hover) backdrop-blur-md  p-3 text-sm font-semibold tracking-wide first:rounded-tl-md first:rounded-bl-md last:rounded-tr-md last:rounded-br-md capitalize"
                  }
          ...
          <tbody className={"tbody"}>
            ...
            {Array.from({ length: 5 }).map((_row, i, arr) => (
              <tr
                key={i}
                className={`block mb-2 text-transparent xl:table-row bg-(--table-row) hover:bg-(--table-row-hover)  backdrop-blur-lg shadow-lg lg:hover:bg-white/30 w-full  ${i === 0 ? "rounded-tl-md" : ""}`}
              >
                {tableDocumentMap.map((h, colIndex) => (
                  <td
                    key={h.key}
                    className={`p-2 ${h.key !== "ip_address" ? "capitalize" : ""} text-center h-12 flex justify-between items-center xl:table-cell ${i === arr.length - 1 && colIndex === 0 ? "rounded-bl-md" : ""} ${i === arr.length - 1 && colIndex === 5 - 1 ? "rounded-br-md" : ""} ${i === 0 && colIndex === 0 ? "rounded-tl-md" : ""} ${i === 0 && colIndex === tableDocumentMap.length - 1 ? "rounded-tr-md" : ""}`}
                  >
                    <span
                      className={
            ...
            {Array.from({ length: 5 }, (_, i) => (
              <tr
                key={i}
                className={`block max-xl:hidden xl:table-row bg-(--table-row) hover:bg-(--table-row-hover)  nth-of-type-[2]:rounded-t-md last:rounded-b-md max-lg:hidden ${tableDocumentMap.length - 1 === 3 ? "text-red-500" : ""}`}
              >
