...
const CurrentSessionCard = ({ session }: { session: CurrentSessionVM }) => {
  ...
  return (
    <div
      className={`absolute top-0 h-95 transition-all duration-500 ease-out transform ${isShowing ? "translate-x-full opacity-100" : "translate-x-0 opacity-0 pointer-events-none"}
  `}
      ref={ref}
    >
      <div
